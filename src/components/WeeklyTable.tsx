...
export default function WeeklyTable({
  ...
  // Group activities directly by Week Number (no Month division)
  ...
  return (
    <div className="bg-white rounded-3xl p-6 sm:p-10 shadow-soft border border-rose-100/60 max-w-5xl mx-auto space-y-8">
      ...
      {/* SEQUENTIAL WEEKLY TABLES (NO MONTH DIVISION) */}
      ...
      {/* OVERALL REFLECTION SECTION (REFLEKSI DITULIS OLEH ADMIN) */}
      <div className="border border-gray-200 rounded-2xl bg-gray-50/60 p-6 space-y-4">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-2">
          <p className="text-xs font-bold text-gray-700 uppercase">Alasan Penilaian & Refleksi:</p>
          {isEditingReflection && isAdmin ? (
            <div className="space-y-3">
              <textarea
                rows={4}
                placeholder="Kosongkan atau tuliskan alasan refleksi di sini..."
                value={reasonInput}
            ...
            <p className="text-xs sm:text-sm text-gray-700 italic leading-relaxed">
              {reflection?.reason && reflection.reason.trim() !== '' ? (
                ...
                <span className="text-gray-400 font-normal">(Belum ada refleksi yang ditulis)</span>
              )}
            </p>
          )}
        </div>
